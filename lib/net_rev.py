...
class Net():
    def __init__(self, pt, model=None, phase = caffe.TEST, noTF=1, accname=None, gt_pt=None, gt_model=None):
        ...
        print("debug!!!!") ############################### print ############################################
        print(pt) ############################### print ############################################
        print(self.net.bottom_names) ############################### print ############################################
        self.pt_dir = pt
    ...
    def infer_pad_kernel(self, W, origin_name):        ########################### NEED TO CONSIDER VARIOUS CASES (NOT ONLY 3) #############
        num_output, _, kernel_h, kernel_w = W.shape
        assert kernel_h % 2 == 1 ###################### changed to consider more various cases - msjeon
        assert kernel_w % 2 == 1 ###################### changed to consider more various cases - msjeon
        pad_h = int(kernel_h / 2)###################### changed to consider more various cases - msjeon
        pad_w = int(kernel_w / 2)###################### changed to consider more various cases - msjeon
    ...
    def R3(self): # TODO: Delete VH and ITQ from R3 to eliminate spatial and channel factorization (tried but failed ㅜㅜ) -by Mario
        ...
        # for NIN model
        alldic = ['conv%d' % i for i in range(1, 4)] + ['cccp%d' % (2*i-1) for i in range(1, 4)]
        pooldic = ['cccp2', 'cccp4', 'cccp6']
        rankdic = {'conv1': 48,
                   'cccp1': 48,
                   'cccp2': 48,
                   'conv2': 128,
        ...
        def getX(name):
            ...
            print(name) ############################### print ############################################
            print("k size:") ############################### print ############################################
            print(self.conv_param_kernel_size(name)) ############################### print ############################################
            temp_ksize = self.conv_param_kernel_size(name)
            return np.rollaxis(x.reshape((-1, temp_ksize, temp_ksize, x.shape[1])), 3, 1).copy() ########################## Dynamically change the kernel size - msjeon
        ...
        for conv, convnext in zip(convs[1:], convs[2:]+[endPool]): # note that we exclude the first conv, conv1_1 contributes little computation -by Mario
            ...
            if True:
                ...
                if 1:
                    ...
                    print(rank) ############################### print ############################################
                    print("ksize:") ############################### print ############################################
                    print(self.conv_param_kernel_size(conv)) ############################### print ############################################
                    V, H, VHr, b = VH_decompose(weights, ksize=self.conv_param_kernel_size(conv), rank=rank, DEBUG=DEBUG, X=X, Y=Y) ######################### Dynamically change the kernel size - msjeon
